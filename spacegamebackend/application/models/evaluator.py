...
class StructureBuildRequirementEvaluator:
    ...
    def evaluate_build(
        ...
        *,
        ...
        evaluators: list[Callable[[], EvalResult]] = [
            partial(self._validate_outpost_placement, structure_template),
            partial(
            ...
            (
                lambda: (
                    self._evaluate_entity_has_enough_resource_slots(structure_template=structure_template)
    ...
    def evaluate_upgrade(
        ...
        *,
        ...
        evaluators: list[Callable[[], EvalResult]] = [
            partial(
                self._validate_outpost_placement,
                structure_to_upgrade.structure_template,
            ),
            partial(
                self._evaluate_components,
    ...
    def _validate_outpost_placement(self, structure_template: StructureTemplate) -> EvalResult:
        if self.x == 0 and self.y == 0 and structure_template.structure_type is StructureType.OUTPOST:
            return EvalResult(False, "Cannot build outpost at 0, 0")
        if self.x != 0 and self.y != 0 and structure_template.structure_type is StructureType.STARTER_OUTPOST:
            return EvalResult(False, "Cannot build starter outpost outside of 0, 0")
        return EvalResult(True)

    def _evaluate_system_has_outpost(self) -> EvalResult:
        solar_system = get_system(x=self.x, y=self.y)
        solar_system_structures = self.user_data_hub.get_structures(entity_id=solar_system.entity_id)
        if not any(
            structure.structure_type in {StructureType.OUTPOST, StructureType.STARTER_OUTPOST}
            for structure in solar_system_structures
        ):
